...
public class BezierCurve extends ParametricCurve {
	...
	/**
	 * 微分したBezier曲線を描画するための点を生成
         * 
	 * @param _parameter パラメータ
	 * @return　微分のBezier曲線の座標点
	 */
    public BezierCurve differentiate() {
        ...
        for (int i = 0; i < n; i++) {
            x = n * (m_cp[i + 1].x() - m_cp[i].x());
            y = n * (m_cp[i + 1].y() - m_cp[i].y());
            z = n * (m_cp[i + 1].z() - m_cp[i].z());
            cp[i] = Point.createXYZ(x, y, z);
        }
        return new BezierCurve(cp, range());
    }

    public double bernstein(int _n, int _i, double _parameter) {
        double B;
        B = comb(_n, _i) * Math.pow(_parameter, _i) * Math.pow((1 - _parameter), _n - _i);
        return B;
    }
     ...
     * @return　組み合わせの計算結果
     */
    public double comb(int _n, int _i) {
        double c_n_i;
        c_n_i = fact(_n) / (fact(_i) * fact(_n - _i));
        return c_n_i;
    }
    ...
    @Override
    public Point evaluate(double _parameter) {
        Point[] cp = m_cp.clone();
        final int degree = cp.length - 1;
        for (int i = 0; i < degree; ++i) {
            for (int j = 0; j < degree - i; ++j) {
    ...
    public Point evaluate(double _parameter, double _time) {
        Point[] cp = m_cp.clone();
        final int degree = cp.length - 1;
        for (int i = 0; i < degree; ++i) {
            for (int j = 0; j < degree - i; ++j) {
        ...
        return Point.createXYZTF(cp[0].x() / _time, cp[0].y() / _time, cp[0].z() / _time, _parameter, 0.0);
    }
     ...
     * @return 指定された Object が、このオブジェクトと 制御点列がまったく同じ BezierCurve である限りtrue
     */
    @Override
