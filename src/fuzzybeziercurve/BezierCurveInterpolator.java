...
public final class BezierCurveInterpolator {
     ...
     */
    public static BezierCurve fit(Point[] _points, int _degree, double _parameterSpan) {
        if (_points == null) {
        ...
        Point[] points = parameterize(_points, _parameterSpan);
        ...
        // 最小自乗法による制御点列の導出
        Point[] cp = calculateControlPoints(wMat, points);

        return BezierCurve.create(cp, Range.zeroToOne());
    }

    /**
     * 正規化時刻列を生成します。 開始時刻を0.0、終了時刻を1.0になるように各点列の時刻を正規化します。
     *
     * @param _points 点列
     * @return 正規化時刻列
     */
    ...
    /**
     * @param _sourceData 点列
     * @param _parameterSpan サンプリング時間
     * @return [0,1]のパラメータを持つ点列
     */
    public static Point[] parameterize(Point[] _sourceData, double _parameterSpan) {
        double max = Double.NEGATIVE_INFINITY;
        Point[] data = new Point[_sourceData.length];
        for (Point p : _sourceData) {
            double t = p.time();
            max = Math.max(max, t);
        }

        for (int i = 0; i < _sourceData.length; ++i) {
            data[i] = Point.createXYZT(_sourceData[i].x(), _sourceData[i].y(), _sourceData[i].z(),
                    1 - (max - _sourceData[i].time()) / _parameterSpan);
        }
