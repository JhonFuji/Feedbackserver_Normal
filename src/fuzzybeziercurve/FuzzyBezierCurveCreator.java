...
public class FuzzyBezierCurveCreator {
    ...
    /**
     * 指定されたベジェ曲線とペン先の速度からファジィベジェ曲線を生成します。
     *
     * @param _bezier ベジェ曲線
     * @param _bezierTime ベジェ曲線の時間間隔
     * @param _vCoeff 速度係数
     * @param _aCoeff 加速度係数
     * @return ファジィベジェ曲線
     */
    public static BezierCurve create(BezierCurve _bezier, double _fittingTimeSpan, double _vCoeff, double _aCoeff) {
        // 入力点列にnullが混入していないかチェック
        if (_bezier == null) {
            throw new IllegalArgumentException();
        }
        BezierCurve vCurve = _bezier.differentiate();
        BezierCurve aCurve = vCurve.differentiate();

        Point origin = Point.createXYZ(0, 0, 0);
        Range range = _bezier.range();
        Point[] points = _bezier.evaluateAll(Math.max((int) Math.ceil(range.length() / 0.01), 2), ParametricEvaluable.EvaluationType.TIME);
        for (int i = 0; i < points.length; ++i) {
            Point p = points[i];
            double time = p.time();
            Point vP = vCurve.evaluate(time, _fittingTimeSpan);
            Point aP = aCurve.evaluate(time, _fittingTimeSpan * _fittingTimeSpan);
        ...
        return create(_bezier, points);
    }

    /**
     * 指定されたベジェ曲線からファジィベジェ曲線を生成します。
     *
     * @param _bezier ベジェ曲線
     * @param _observations 評価点
     * @return ファジィベジェ曲線
     */
    public static BezierCurve create(BezierCurve _bezier, Point[] _observations) {
        if (_bezier == null) {
        ...
        // 次数
        int degree = _bezier.degree();
        double[] times = BezierCurveInterpolator.createNormalizedTimes(_observations);
        // 重み行列
        Matrix weightMatrix = BezierCurveInterpolator.createWeightMatrix(times, degree);

        // 非負制約条件下の最小自乗法により，ファジネスを求める
        double[] fuzzinessElements = nnls(weightMatrix, observations);

        // ファジネスを制御点に付加
        Point[] cp = _bezier.controlPoints();
        for (int i = 0; i < cp.length; ++i) {
            Point p = cp[i];
            cp[i] = Point.createXYZTF(p.x(), p.y(), p.z(), p.time(), fuzzinessElements[i]);
        }

        return BezierCurve.create(cp, _bezier.range());
    }
     ...
     * @param _observation observation
     * @return 最小自乗問題の解
     */
